...
- 当前 `string.find` / `string.match` / `string.gsub` 已支持纯文本子串处理；其中 `find` / `match` 支持 Lua 风格起始下标，`gsub` 支持字符串替换值、返回替换后的字符串和替换次数；Lua 5.1 pattern / capture / function-table replacer 语义仍未实现。
...
- 当前已内置 `print`、`clock_ms`、`type`、`tostring`、`tonumber`、`select`、`unpack`、`assert`、`error`、`pcall`、`xpcall`、`next`、`pairs`、`ipairs`、`rawequal`，以及最小 `table.getn` / `table.maxn` / `table.insert` / `table.remove` / `table.concat` / `table.sort`、`math.abs` / `math.floor` / `math.ceil` / `math.max` / `math.min` / `math.sqrt` / `math.pow` / `math.random` / `math.randomseed` / `math.log` / `math.exp` / `math.sin` / `math.cos` 和 `string.find` / `string.match` / `string.gsub` / `string.len` / `string.sub` / `string.lower` / `string.upper` / `string.rep` / `string.reverse` / `string.byte` / `string.char`；这仍只是较小的基础内建子集。
